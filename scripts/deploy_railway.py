...
""", {"input": {
        ...
        print(f"  {name}: set RAILWAY_RUN_CMD fallback")
...
for name in ["backend", "celery", "celery-beat"]:
    ...
    cli(
        ...
        f"railway up --service {name} --detach 2>&1 || true",
        cwd=backend_dir, check=False
    )

print("\n  Deploying frontend...")
cli(
    f"RAILWAY_SERVICE_ID={SVCS['frontend']} railway up --detach 2>&1 || "
    f"railway up --service frontend --detach 2>&1 || true",
...
""", {"serviceId": svc_id, "environmentId": ENV_ID}, soft=True)
    if d and d.get("serviceDomainCreate"):
        ...
        print(f"  {name}: {url}")
        if name == "backend": BACKEND_URL = url
        if name == "frontend": FRONTEND_URL = url
    else:
        print(f"  {name}: domain will appear in Railway dashboard after first build")
...
""", {"input": {
        ...
        "variables": {"NEXT_PUBLIC_API_URL": BACKEND_URL,
                      "NEXT_PUBLIC_REQUIRE_AUTH": "false", "PORT": "3000"},
    }})
    print(f"  Frontend NEXT_PUBLIC_API_URL updated to {BACKEND_URL}")
...
print("="*60)
print(f"""
  Railway Dashboard (watch build progress here):
  https://railway.app/project/{PROJECT_ID}

  Services are building now (3-5 min each).
""")
if FRONTEND_URL: print(f"  Frontend: {FRONTEND_URL}")
else: print("  Frontend URL: check Railway dashboard after builds complete")
if BACKEND_URL: print(f"  Backend:  {BACKEND_URL}")
else: print("  Backend URL: check Railway dashboard after builds complete")
print(f"""
  NEXT STEP — add your Anthropic key in Railway:
  https://railway.app/project/{PROJECT_ID}
  -> backend service -> Variables -> ANTHROPIC_API_KEY
""")
if BACKEND_URL:
    print(f"""  Then create your first API key:
