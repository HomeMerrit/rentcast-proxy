...
import type { Agent, AGUIEvent, AGUIEventType } from "@/types/agent";
...
const EVENT_TEXT: Partial<Record<AGUIEventType, string>> = {
  ...
  TOOL_CALL_RESULT: "text-amber-300",
...
function eventDescription(ev: AGUIEvent): string {
  const d = ev.data;
  switch (ev.type) {
    case "RUN_STARTED":
      return "Task started";
    ...
    case "RUN_ERROR":
      return truncate((d.message as string) ?? "Error occurred");
    case "TEXT_MESSAGE_START":
      return "Message started";
    case "TEXT_MESSAGE_CONTENT": {
      const delta = (d.delta as string) ?? "";
      return delta.trim() ? truncate(delta) : "…";
    }
    case "TEXT_MESSAGE_END":
      return "Message complete";
    case "TOOL_CALL_START":
      return `Tool: ${(d.tool_name as string) ?? "unknown"}`;
    case "TOOL_CALL_ARGS": {
      const raw = typeof d.args === "string" ? d.args : JSON.stringify(d.args ?? {});
      return truncate(raw);
    }
    case "TOOL_CALL_END":
    ...
    case "STATE_SNAPSHOT":
      return `Status → ${(d.status as string) ?? "updated"}`;
    case "STATE_DELTA":
      return d.status
        ? `Status → ${d.status as string}`
        : truncate(JSON.stringify(d));
    case "CUSTOM":
...
export function AgentProfile({ agent }: Props) {
  ...
  const recentEvents = [...stream.events].reverse().slice(0, 20);
    ...
    <div className="min-h-screen bg-gray-950">
      ...
      <div className="max-w-4xl mx-auto px-6 py-8 space-y-6">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl overflow-hidden">
            ...
            <span className="relative flex flex-shrink-0">
              ...
              <span
                className={`relative inline-flex rounded-full w-2 h-2 ${
                  stream.isConnected ? "bg-emerald-500" : "bg-gray-600"
                }`}
              />
            </span>
            <h2 className="text-sm font-semibold text-gray-200">Live Activity</h2>
            <span className="text-xs text-gray-600">
              {stream.isConnected ? "Connected" : "Connecting…"}
            </span>
            {stream.lastActivity && (
              <span className="ml-auto text-xs text-gray-700">
                Last: {timeLabel(stream.lastActivity)}
              </span>
            )}
          </div>
          ...
          {/* Active tool chip */}
          {stream.currentTool && !stream.currentMessage && (
            <div className="px-5 py-3 border-b border-gray-800">
              <span className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 text-amber-400 text-xs font-medium rounded-full border border-amber-500/25">
                ...
                Using tool: {stream.currentTool}
              </span>
            </div>
          )}

          {/* Static current_task (only when no live message/tool) */}
          {!stream.currentMessage && !stream.currentTool && agent.current_task && (
            <div className="px-5 py-3 border-b border-gray-800">
